...
fn main() {
    ...
    loop_with_report(&mut |_dt: f64| {
        ...
        let mut q = CmdQueue::new(surface, &ass);
        {
            ...
            for i in 0..5{
                for j in 0..5{
                    q.sprite(vec2(i * 300, j * 300), 2, sp1);
                    q.sprite(vec2(i * 350, j * 320), 2, sp2);
                }
            }
            ...
            q.rect(vec2(W/2 + 480,H/2 + 480), vec2(80, 80), 3).with_color(1.0,1.0,0.0,1.0);
            q.rect(vec2(W/2 + 500,H/2 + 500), vec2(40, 40), 4).with_color(0.0,1.0,1.0,1.0).with_border(3).with_color(1.0, 0.0, 0.0, 1.0);
        }
        q.done();
