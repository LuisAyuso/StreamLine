...
impl TextDraw {
    pub fn new<F>(f: &F) -> TextDraw
        where F: glium::backend::Facade
    {

        let sys = glium_text::TextSystem::new(f);
    ...
    pub fn add_font<F, FIO>(&mut self, f: &F, font: FIO) -> u32
        where F: glium::backend::Facade,
              FIO: io::Read
    {
        let font = glium_text::FontTexture::new(f, font, 24).unwrap();
